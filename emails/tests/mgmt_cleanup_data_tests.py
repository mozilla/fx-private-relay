"""Tests for emails/management/commands/cleanup_data.py"""
...
def test_all_server_storage(all_users_have_server_storage) -> None:
    """The command detects when all users have server storage."""
    out = StringIO()
    call_command(COMMAND_NAME, stdout=out)
    output = out.getvalue()
    expected = """\
...
def test_data_cleared(users_need_data_cleared) -> None:
    """The command can clear user data when they request no server storage."""
    out = StringIO()
    call_command(COMMAND_NAME, "--clear", stdout=out)
    output = out.getvalue()
    expected = """\
