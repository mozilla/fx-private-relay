...
class SNSNotificationTest(SNSNotificationTestBase):
    """Tests for _sns_notification for incoming emails"""
...
class SNSNotificationReplyTest(SNSNotificationTestBase):
...
class SNSNotificationValidUserEmailsInS3Test(TestCase):
    ...
    def test_relay_address_disabled_email_in_s3_deleted(self) -> None:
        ...
        assert (event := get_glean_event(caplog)) is not None
        expected = self.get_expected_event(event["timestamp"], "block_all")
        assert event == expected
